...
.ok
	ldh [hCurSelectionItem], a ; number of cards
...
; deck indices that are in the Deck.
	...
	ld de, wDuelTempList
	ld c, 0
	call PickRandomBasicEnergyCardFromDeck
...
; check how many were found
	...
	jr nz, .found
	; return if none found
	ldtx hl, ThereWasNoEnergyText
	call DrawWideTextBox_WaitForInput
	ld a, 1
.found
	; pick 1 card that was found
	ld a, 1
	cp c 
	jr c, .ok 
	ld a, c
...
; if none are found, return carry.
PickRandomBasicEnergyCardFromDeck:
	call CreateDeckCardList
	ret c ; return if empty deck
	ld hl, wDuelTempList
;	call ShuffleCards
